...
for f in $RESULT; do
    ...
    strList="${strList}<h4>${title/name:/''}</h4><p>${description/\# description:/''}</p><p>↳ 🔗 <a href='${URL}${f}' title='open the action'>${f}</a> (<a href='${RAW_URL}${f}' title='open the action'>raw</a>)</p><hr>${${BACK_TO_TOP}}"
done
